...
export class GrpcServer {
  ...
  private async loadDynamicModules(): Promise<void> {
    ...
    const rootPath = isDist ? join(__dirname, '..', '..') : join(__dirname, '..');
    
    const commonIncludeDirs = [
      isDist ? join(rootPath, 'dist', 'core', 'proto') : join(__dirname, 'proto')
    ];
    ...
    const dynamicProtosDir = isDist ? join(rootPath, 'src', 'dynamic-modules', 'protos') : join(rootPath, 'dynamic-modules', 'protos');
    const dynamicAdaptersDir = join(__dirname, '..', 'dynamic-modules', 'adapters', 'grpc');

    if (existsSync(dynamicProtosDir)) {
      const files = readdirSync(dynamicProtosDir).filter(f => f.endsWith('.proto'));
      for (const file of files) {
        const entityName = file.replace('.proto', '');
        const protoPath = join(dynamicProtosDir, file);
        const adapterPath = join(dynamicAdaptersDir, `${entityName}.${isDist ? 'js' : 'ts'}`);
